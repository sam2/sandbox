...
Void FlxKeyboard_obj::onKeyUp( ::flash::events::KeyboardEvent FlashEvent){
{
		...
		int c = FlashEvent->keyCode;		HX_STACK_VAR(c,"c");
		HX_STACK_LINE(387)
		if (((bool((::flixel::FlxG_obj::game->debugger != null())) && bool(this->inKeyArray(::flixel::FlxG_obj::debugger->toggleKeys,c))))){
			HX_STACK_LINE(389)
	...
	HX_STACK_LINE(253)
	{
		...
		while(((_g < KeyArray->__Field(HX_CSTRING("length"),true)))){
			...
			if (((k != null()))){
				...
				HX_STACK_LINE(276)
				::flixel::FlxG_obj::log->error(((HX_CSTRING("Invalid Key: `") + ::Std_obj::string(key)) + HX_CSTRING("`. Note that function and numpad keys can only be used in flash and js.")));
			}
		}
	}
	HX_STACK_LINE(282)
