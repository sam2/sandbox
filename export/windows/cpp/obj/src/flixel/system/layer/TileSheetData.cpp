...
::flixel::system::layer::frames::FlxFrame TileSheetData_obj::addSpriteSheetFrame( ::flash::geom::Rectangle rect,::flash::geom::Point point){
	...
	HX_STACK_ARG(point,"point");
	struct _Function_1_1{
		inline static ::String Block( ::flash::geom::Rectangle &rect,::flash::geom::Point &point){
	...
	HX_STACK_LINE(168)
	::String key = _Function_1_1::Block(rect,point);		HX_STACK_VAR(key,"key");
	HX_STACK_LINE(169)
	if ((this->containsFrame(key))){
		HX_STACK_LINE(170)
		return this->flxFrames->get(key);
...
bool TileSheetData_obj::containsSpriteSheetFrames( int width,int height,int startX,int startY,int endX,int endY,int xSpacing,int ySpacing,Float pointX,Float pointY){
	...
	struct _Function_1_1{
		inline static ::String Block( int &width,int &xSpacing,int &endY,int &endX,int &startY,int &ySpacing,int &height,int &startX,Float &pointY,Float &pointX){
	...
	HX_STACK_LINE(159)
	::String key = _Function_1_1::Block(width,xSpacing,endY,endX,startY,ySpacing,height,startX,pointY,pointX);		HX_STACK_VAR(key,"key");
	HX_STACK_LINE(160)
