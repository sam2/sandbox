...
{
HX_STACK_PUSH("GameClass::new","GameClass.hx",9);
{
	HX_STACK_LINE(10)
	...
	HX_STACK_LINE(13)
	Float ratioX = (Float(stageWidth) / Float((int)960));		HX_STACK_VAR(ratioX,"ratioX");
	HX_STACK_LINE(14)
	Float ratioY = (Float(stageHeight) / Float((int)540));		HX_STACK_VAR(ratioY,"ratioY");
	HX_STACK_LINE(15)
	Float ratio = ::Math_obj::min(ratioX,ratioY);		HX_STACK_VAR(ratio,"ratio");
	HX_STACK_LINE(17)
	int fps = (int)60;		HX_STACK_VAR(fps,"fps");
	HX_STACK_LINE(19)
	super::__construct(::Math_obj::ceil((Float(stageWidth) / Float(ratio))),::Math_obj::ceil((Float(stageHeight) / Float(ratio))),hx::ClassOf< ::PlayState >(),ratio,fps,fps,null());
}
;
	return null();
}
